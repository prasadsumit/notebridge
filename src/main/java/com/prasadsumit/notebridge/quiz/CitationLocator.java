...
final class CitationLocator {
    ...
    private static String nearestHeading(NoteChunk chunk) {
        ...
        if (content == null || chunk.getContent() == null)
            return null;

        int position = content.indexOf(chunk.getContent());

        if (position < 0 || content.indexOf(chunk.getContent(), position + 1) >= 0)
            return null;
    ...
    private static LocatedHeading lastMatch(Matcher matcher) {
        ...
        while (matcher.find())
            value = new LocatedHeading(matcher.start(), matcher.group(1).trim());
